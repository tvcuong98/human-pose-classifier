...
def get_feedback_loss(args,classification_model,data_real,generator_output,labels,current_epoch,device=torch.device("cuda"),criterion=nn.CrossEntropyLoss()):
    """
    INPUT:
    ...
    # fake_2d_pose - > classification model -> output 9 class
    fake_loss = get_classification_loss(generator_output,classification_model,labels,criterion,device)
    hardratio = update_hardratio(args.hardratio_bl_s, args.hardratio_bl, current_epoch, args.epochs)
    # get feedback loss
    pos_fake_loss_blToReal  = fix_hardratio(args.hardratio_std_bl, hardratio,
                                             fake_loss, real_loss,
                                             args.gloss_factordiv_bl, args.gloss_factorfeedback_bl, tag='bl')
    feedback_loss = pos_fake_loss_blToReal
