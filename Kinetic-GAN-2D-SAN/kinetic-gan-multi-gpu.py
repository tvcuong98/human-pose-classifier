...
# make the optimizer multi-gpu
## make the model multi_gpu 
if NUM_GPU > 1:
    optimizer_G = torch.optim.Adam(generator.parameters(), lr=opt.lr*NUM_GPU, betas=(opt.b1, opt.b2))
    optimizer_D = torch.optim.Adam(discriminator.parameters(), lr=opt.lr*NUM_GPU, betas=(opt.b1, opt.b2))
...
for epoch in range(opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        with torch.cuda.amp.autocast(enabled=True):
            ...
            # Real actions
            real_validity = discriminator(real_imgs, labels,False)
            # Fake actions
            fake_validity = discriminator(fake_imgs, labels,False)
